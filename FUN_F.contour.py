...
def contour_image(image_name, backg=10):
    ...
    if cv2.contourArea(cnt1) >= gnum_pixels * 0.96:
        adaptive_thresholded_image = adaptive_thresholded_image[:adaptive_thresholded_image.shape[0]-9, :]
    ...
    if cv2.contourArea(cnt1) >= gnum_pixels / 5:
        ...
        contoured = four_point_transform(adaptive_thresholded_image0, approx.reshape(4, 2) * 1)
    else:
